...
def init_sim(model, mesh, seed, param):
    ...
    sim.setMembVolRes('membrane', param['R_A']*1.0)
    sim.setMembCapac('membrane', param['C_M']/area_mesh_factor)

    vs = list(range(mesh.nverts))
    random.shuffle(vs)
    
    for v in range(mesh.nverts):
        if v%2: sim.setVertVolRes(vs[v], param['R_A']*1.01)
        else: sim.setVertVolRes(vs[v], param['R_A']*0.99)
