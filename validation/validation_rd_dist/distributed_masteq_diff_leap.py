...
class TestMasteqDiffLeap(unittest.TestCase):

    def test_masteq_diff_leap(self):
        "Reaction-diffusion - Production and second order degradation (Parallel DistTetOpSplit with leaping)"
        ...
        sim = Simulation('DistTetOpSplit', mdl, mesh, rng, SSAMethod=SSAMethod.RLEAPING, searchMethod=NextEventSearchMethod.RLEAPING)
...
def suite():
    all_tests = []
    all_tests.append(unittest.TestLoader().loadTestsFromTestCase(TestMasteqDiffLeap))
