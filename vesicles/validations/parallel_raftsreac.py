...
class VesicleRaftSReac(unittest.TestCase):
    def test_raftsreac_n8(self):
        ########################################################################
        ...
        rs_soAA = rs.memb.raft.A_soAA.Count / \
            volfact << rs.comp.B_soAA.Conc << rs.memb.raft.C_soAA.Count / volfact
        rs_soAB = rs.memb.raft.A_soAB.Count / volfact << rs.comp.B_soAB.Conc

        sim.toSave(rs_foi, rs_for, rs_soAA, rs_soAB, dt=DT)

        with HDF5Handler(os.path.join(FILEDIR, 'data/raftsreac_test')) as hdf:
            sim.toDB(hdf, f'raftsreac')
        ...
        if MPI.rank == 0:
            with HDF5Handler(os.path.join(FILEDIR, 'data/raftsreac_test')) as hdf:
                rs_foi, rs_for, rs_soAA, rs_soAB = hdf['raftsreac'].results
                tpnts = rs_for.time[0]
                ...
                fig.savefig(os.path.join(FILEDIR, "plots/raftsreac.pdf"), dpi=300, bbox_inches='tight')
                plt.close()
...
def suite():
    all_tests = []
    all_tests.append(unittest.TestLoader().loadTestsFromTestCase(VesicleRaftSReac))
