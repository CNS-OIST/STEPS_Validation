...
class VesicleVesReacWeight(unittest.TestCase):
    def test_vesreac_weight_n4(self):
        ########################################################################
        ...
        if MPI.rank == 0:
            with HDF5Handler(filePrefix) as hdf:
                ...
                fig.savefig(os.path.join(FILEDIR, "plots/vesreac_weight.pdf"), dpi=300, bbox_inches='tight')
                plt.close()
...
def suite():
    all_tests = []
    all_tests.append(unittest.TestLoader().loadTestsFromTestCase(VesicleVesReacWeight))
