...
class VesicleRaftSReacWeight(unittest.TestCase):
    def test_raftsreac_weight_n8(self):
        ########################################################################
        ...
        if MPI.rank == 0:
            with HDF5Handler(filePrefix) as hdf:
                rs_foi, rs_for, rs_soAA, rs_soAB = hdf['raftsreac_weight'].results
                tpnts = rs_for.time[0]
                ...
                fig.savefig(os.path.join(FILEDIR, "plots/raftsreac_weight.pdf"), dpi=300, bbox_inches='tight')
                plt.close()
...
def suite():
    all_tests = []
    all_tests.append(unittest.TestLoader().loadTestsFromTestCase(VesicleRaftSReacWeight))
